...
describe("Policy guardian recovery script", function () {
    ...
    it("Scripts should recover the Safe from the policy guardian with the user initiating recovery", () => recoverSafeFromPolicyGuardian());
    it("Scripts should recover the Safe from the policy guardian with Waymont initiating recovery", () => recoverSafeFromPolicyGuardian(true, false));
    it("Scripts should recover the Safe from the policy guardian with Waymont permanently initiating recovery", () => recoverSafeFromPolicyGuardian(true, true));
});

// Recover Safe from policy guardian
async function recoverSafeFromPolicyGuardian(waymontInitiatedRecovery, waymontInitiatedRecoveryPermanently) {
    // Get signers
    const [relayer] = await ethers.getSigners();
